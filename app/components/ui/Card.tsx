...
export default function Card({ href, icon, label, sublabel }: CardProps) {
  return (
    <Link
      href={href}
    ...
    >
      <div className="flex items-center justify-center w-14 h-14 rounded-full bg-action-blue/10 text-action-blue shrink-0">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-lg font-semibold text-primary-navy">{label}</div>
        {sublabel && (
          <div className="text-base text-text-secondary truncate">
            {sublabel}
          </div>
        )}
      </div>
